...
From APIM 3.7, you can use a custom prefix for your collections name.

=== On a new installation

You just have to modify `gravitee.yml` (for the gateway and the console) to configure `management.jdbc.prefix` and `ratelimit.jdbc.prefix`. Default value is empty.
...
This script renames all the collections by adding the prefix and rateLimitPrefix you choose.

Then, modify `gravitee.yml` files to configure `management.mongodb.prefix` and `ratelimit.mongodb.prefix`.
